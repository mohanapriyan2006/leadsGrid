...
function useRipple() {
  return (e: React.MouseEvent<HTMLElement>) => {
    ...
    const diam = Math.max(el.clientWidth, el.clientHeight);
    const rect = el.getBoundingClientRect();
    circle.className = "ripple-circle";
    Object.assign(circle.style, {
      width: `${diam}px`, height: `${diam}px`,
      left: `${e.clientX - rect.left - diam / 2}px`,
      top:  `${e.clientY - rect.top  - diam / 2}px`,
    });
...
const SourceIcon = ({ source }: { source: string }) => {
  ...
  if (source === "reddit") return (
    <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor">
      ...
      <path fill="var(--bg-card)" d="M15.5 10.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5S14 12.83 14 12s.67-1.5 1.5-1.5zm-7 0c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5S7 12.83 7 12s.67-1.5 1.5-1.5zM12 16c2 0 3.5-1 3.5-1s-1 2-3.5 2-3.5-2-3.5-2 1.5 1 3.5 1z"/>
    </svg>
  );
  return (
    <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor">
...
function SkeletonCard() {
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-card)", padding: "18px 20px" }}>
...
export const LeadsPage = () => {
  ...
  const [sources, setSources]         = useState<string[]>(["linkedin", "twitter"]);
  ...
  return (
    <div className="leads-page">
      ...
      <div className="lp-filters">
        ...
        {(["linkedin", "twitter", "reddit"] as const).map((s) => (
        ...
        <select className="industry-select" value={industry} onChange={(e) => setIndustry(e.target.value)}>
          {["Software & SaaS", "FinTech", "HealthTech", "eCommerce", "Enterprise"].map((i) => (
            <option key={i} value={i}>{i}</option>
          ))}
        </select>

        <button className="clear-btn" onClick={() => { setScoreMin(0); setSources(["linkedin", "twitter", "reddit"]); }}>
          Clear all
        </button>
      </div>
          ...
          <div className="leads-list">
            {isLoading
              ...
              : filteredLeads.map((lead: Lead, idx: number) => {
                  ...
                  const source = (lead as any).source ?? "linkedin";
                  const barW   = `${score}%`;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="lead-card-top">
                        <div className="lead-avatar">{initials((lead as any).name ?? "?")}</div>
                        <div className="lead-identity">
                          <div className="lead-name">{(lead as any).name ?? lead.id}</div>
                          <div className="lead-title">{(lead as any).title ?? "Unknown Role"}</div>
                        </div>
                        <div className="lead-score-block">
                          ...
                          <div className="lead-score-bar-wrap">
                            <div
                              ...
                              style={{ "--bar-w": barW, width: barW, background: scoreColor(score) } as React.CSSProperties}
                            />
                          </div>
                        </div>
                      </div>
          ...
          {selectedLead ? (
            <div className="selected-lead-summary">
              <div className="lead-avatar" style={{ width: 36, height: 36, fontSize: 13 }}>
                {initials((selectedLead as any).name ?? "?")}
              </div>
              <div className="selected-lead-summary-info">
                <h4>{(selectedLead as any).name ?? selectedLead.id}</h4>
                <p>{(selectedLead as any).title ?? "Unknown Role"}</p>
              </div>
              <div className="selected-lead-summary-score">{(selectedLead as any).score ?? "—"}</div>
            </div>
          ) : (
