...
const iconMap: Record<string, (isActive: boolean) => React.ReactNode> = {
  "Dashboard": (isActive) => (
    <div className={`flex items-center justify-center w-8 h-8 rounded-lg transition-all duration-200 ${isActive ? "bg-accent/20 text-accent" : "bg-surface-tertiary/50  "}`}>
  ...
  return (
    <aside className={`hidden w-72 border-r border-accent/10 bg-gradient-to-b from-surface-secondary/90 via-surface-secondary/80 to-surface/90 p-5 backdrop-blur-glass md:block ${className}`.trim()}>
      ...
      <div className="flex items-center gap-3 mb-4" onClick={() => navigate("/")} style={{ cursor: "pointer" }}>
        ...
        <div>
          <span className="text-xl uppercase tracking-[0.10em]  ">LeadsGrid</span>
          <p className="text-[10px]   uppercase tracking-widest">Lead Intelligence</p>
        </div>
      </div>

      {/* Navigation - Pro Level with Icons */}
      <nav className="space-y-1">
        <p className="text-[10px] uppercase tracking-[0.2em]   mb-3 px-2 font-medium">Menu</p>
