  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-white/40">{lead.timestamp}</span>
      </div>
      <p className="mb-2 text-sm font-medium text-white/90">"{lead.title}"</p>
      <div className="flex items-center justify-between">
        <span className={`text-xs font-medium ${sourceColors[lead.source] ?? "text-white/50"}`}>
          {lead.source}
        </span>
